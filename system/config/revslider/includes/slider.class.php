	 ...
	 */
		...
		//	var_dump($zip);die();
			...
			$usepcl = true;
              ...
              //  var_dump($zip);die();
		//var_dump(RevSliderGlobals::$uploadsUrlExportZip);die();
		//add svg to the zip
		if(!empty($usedSVG)){
		...
		if(!empty($usedImages)){
			...
			$cont_url_no_www = str_replace('www.', '', $upload_dir_multisiteless_baseurl); 
                  //      var_dump($upload_dir_multisiteless);var_dump($upload_dir_multisiteless_baseurl);die();
			...
			if(!class_exists("ZipArchive")){
					...
					$importZip = false;
                                        $unzipfile = false;
                                            ...
                                            if ($zip->extract(PCLZIP_OPT_PATH,$rem_path) == 0) {
...
//					}
				...
				if(!empty($navigations)){
					...
					//dmp(__("navigations imported!",'revslider'));
                            ...
                            $importZip = true; //raus damit..
                            if(file_exists($rem_path."slider_export.txt")){
                                   $content =  @file_get_contents($rem_path."slider_export.txt");
                                }
                                if($content == ''){
                                    RevSliderFunctions::throwError(__('slider_export.txt does not exist!', 'revslider'));
				} 
                        
                                 if(file_exists($rem_path."custom_animations.txt")){
                                   $animations =  @file_get_contents($rem_path."custom_animations.txt");
                                 }else{
                                    $animations = '';
                                 }
                                 if(file_exists($rem_path."dynamic-captions.css")){
                                   $dynamic =  @file_get_contents($rem_path."dynamic-captions.css");
                                 }else{
                                    $dynamic = '';
                                 }
                                 if(file_exists($rem_path."static-captions.css")){
                                   $static =  @file_get_contents($rem_path."static-captions.css");
                                 }else{
                                    $static = '';
                                 }
                                 if(file_exists($rem_path."navigation.txt")){
                                   $navigations =  @file_get_contents($rem_path."navigation.txt");
                                 }else{
                                    $navigations = '';
                                 }
                                  if(file_exists($rem_path."info.cfg")){
                                   $uid_check =  @file_get_contents($rem_path."info.cfg");
                                 }else{
                                    $uid_check = '';
                                 }
                                 if(file_exists($rem_path."version.cfg")){
                                   $version_check =  @file_get_contents($rem_path."version.cfg");
                                 }else{
                                    $version_check = '';
                                 }
                                if($is_template !== false){
...
//	
	...
	public function getFirstSlideIdFromGallery(){
		...
		$arrSlideRecords = $this->db->fetch(RevSliderGlobals::$table_slides,$this->db->prepare("slider_id = %s", array($this->id)),"slide_order",'',' LIMIT 0,1');
                ...
                if($arrSlideRecords ==null){
                    $arrSlideRecords = array();
                }
