...
class RevSliderTemplate {
	...
	public function _download_template($uid){
		...
		// Check folder permission and define file location
 		if(wp_mkdir_p( $upload_dir.$this->templates_path ) ) { //check here to not flood the server
			...
			if($request !=null && !empty($request)) {
				if($response = $request['body']) {
					if($response !== 'invalid'){
						...
						$file = $upload_dir. $this->templates_path . '/' . $uid.'.zip';
						@mkdir(dirname($file));
                                           //     var_dump($response);die('kkk'); 
	...
	public function _delete_template($uid){
		...
		// Check folder permission and define file location
		if( wp_mkdir_p( $upload_dir.$this->templates_path ) ) {
			$file = $upload_dir. $this->templates_path . '/' . $uid.'.zip';
	...
	public function _get_template_list($force = false){
		...
		if($last_check == false){ //first time called
			...
			update_option('revslider-templates-check',  time());
                ...
                $diff = (int)time() - (int)$last_check; 
                
		// Get latest Templates
		if($diff > 345600 || $force == true){ //4 days
	...
	private function update_template_list(){
		...
		$new = get_option('rs-templates-new', false);
                if(!is_array($new) && !empty($new)){
                   $new = (array)json_decode($new,true); 
                }
			...
			if(empty($cur)){
				...
				if(isset($new['slider']) && is_array($new['slider'])){
					...
					$cur['slides'] = $new['slides']; // push always all slides
                        ...
                        $allowed_templates = get_option('allowed_templates',array()); 
                        if(!empty($allowed_templates)){
                            $allowed_templates = json_decode($allowed_templates,true);
                        }
                          
                         $count_slider = 0;
                         foreach($cur['slider'] as $template_slider){
                            if(in_array($template_slider['alias'],$allowed_templates)){
                                unset($cur['slider'][$count_slider]);
                            }
                            $count_slider++;
                         }
                         $count_slides = 0;
                         foreach($cur['slides'] as $template_slides){
                            if(in_array($template_slides,$allowed_templates)){
                                unset($cur['slides'][$count_slides]);
                            }
                            $count_slides++;
                         }
	...
	public function remove_is_new($uid){
		$cur = get_option('rs-templates', array());
                if(!is_array($cur) && !empty($cur)){
                   $cur = (array)json_decode($cur,true); 
                }
		if(isset($cur['slider']) && is_array($cur['slider'])){
			foreach($cur['slider'] as $ck => $c){
				if($c['uid'] == $uid){
		...
		if(!empty($templates) && is_array($templates)){
			...
			if(!empty($templates['slider']) && is_array($templates['slider'])){
				foreach($templates['slider'] as $key => $temp){
					...
					// Check folder permission and define file location
					if( wp_mkdir_p( $upload_dir.$this->templates_path ) ) {
						$file = $upload_dir. $this->templates_path . '/' . $temp['img'];
						...
						if((!file_exists($file) && !file_exists($file_plugin)) || isset($temp['push_image'])){
							if($curl !== false){
								$image_data = @wp_remote_fopen($this->templates_url.$this->templates_server_path.$temp['img'],array()); // Get image data
			...
			if(!empty($templates['slides']) && is_array($templates['slides'])){
				foreach($templates['slides'] as $key => $temp){
					foreach($temp as $k => $tvalues){
						...
						// Check folder permission and define file location
						if( wp_mkdir_p( $upload_dir.$this->templates_path ) ) {
							$file = $upload_dir . $this->templates_path . '/' . $tvalues['img'];
							...
							if((!file_exists($file) && !file_exists($file_plugin)) || isset($reload[$key])){ //update, so load again
								if($curl !== false){
									$image_data = @wp_remote_fopen($this->templates_url.$this->templates_server_path.$tvalues['img'],array()); // Get image data
									if( isset($image_data['body']) && isset($image_data['response']) && isset($image_data['response']['code']) && $image_data['response']['code'] == '200'){
										$image_data = $image_data['body'];
									}else{
										$image_data = false;
	...
	public function getThemePunchTemplateSliders(){
		...
		$defaults = get_option('rs-templates', array());
                ...
                if(!is_array($defaults) && !empty($defaults)){
                   $defaults = (array)json_decode($defaults,true); 
                }
		$defaults = (isset($defaults['slider'])) ? $defaults['slider'] : array();
              //  var_dump($defaults);die();
               // $defaults = array();
                // this line is added on custom
              //  $forcfully_get_list = $this->_get_template_list(true);
		//var_dump($forcfully_get_list);die();
	...
	public function _check_file_path($image, $url = false){
		...
		$file = $upload_dir . $this->templates_path . '/' . $image;
		$file_plugin = RS_PLUGIN_PATH . $this->templates_path_plugin . '/' . $image;
//                var_dump($this->templates_path);
//		var_dump($file);var_dump($file_plugin);
              
		if(file_exists($file)){ //downloaded image first, for update reasons
			if($url){
				$image = $upload_url . $this->templates_path . '/' . $image;
			}else{
				$image = $upload_dir . $this->templates_path . '/' . $image; //server path
			}
