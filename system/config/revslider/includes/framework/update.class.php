...
class RevSliderUpdate {
	...
	private $remote_url			= 'http://updates.themepunch.tools/check_for_updates.php';
        private $remote_url_info_sds	= 'https://updates.themepunch.tools/revslider-opencart/revslider.php';
	private $remote_url_info	= 'http://updates.themepunch.tools/revslider-opencart/revslider.php';
	private $remote_temp_active	= 'http://updates.themepunch.tools/temp_activate.php';
	private $plugin_slug		= 'revslider-opencart';
	...
	protected function _check_updates() {
		...
		$code = get_option('revslider-code', ''); 
                $request_sds = wp_remote_post($this->remote_url_info_sds, array(
                    ...
                    'body' => array(
                        'item' => urlencode('revslider-opencart'),
	...
	public function update_revslider(){
         //   die('please configure before release');
            global $wp_version;
            ...
            if($url !=false){
                $get = wp_remote_post($url, array(
                        ...
                        $file = $upload_dir. 'revjoomlaupdate.zip';

                        @mkdir(dirname($file));
                        $ret = @file_put_contents( $file, $get['body'] ); 

                        $d_path = get_http_catalog();
