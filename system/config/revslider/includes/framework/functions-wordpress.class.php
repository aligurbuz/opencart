...
//		}
	 ...
	 */
         public static function import_media_img_old($file_url, $folder, $filename){
                    $wpdb = rev_db_class::rev_db_instance();
                  ...
                  //  var_dump($subdir_filename);die();
                    ...
                    if(!empty($mysqli) && is_numeric($imgid)){
                        ...
                        foreach($fsizes as $sizew=>$sizeh){
                            ...
                            $res_img = new SdsRevToolImage();
                            if(++$count > 1){
                                $res_img->resize($folder.$subdir_filename, $sizew, $sizeh,$folder.$suburl.$newfilename);
                            }else{
                                $res_img->resize($folder.$subdir_filename, $sizew, $sizeh,$folder.$suburl.$newfilename);
                            }
                        }
...
// 
                public static function import_media_old($file_url,$alias=''){
 ...
 public static function import_media($file_url,$alias=''){
	...
	public static function getImageUrlFromPath($pathImage){
		...
		$urlImage = get_mainsite_url().$pathImage;
		return($urlImage); 
	}
	...
	public static function getImageUrlFromPathOc($pathImage){
		//protect from absolute url
		$pathLower = strtolower($pathImage);
		if(strpos($pathLower, "http://") !== false || strpos($pathLower, "https://") !== false || strpos($pathLower, "www.") === 0)
			return($pathImage);
...
//		return($langTag);
	...
	public static function get_image_id_by_url($image_url) {
		...
		if ( 0 == $attachment_id ){ //try to get it old school way
			...
			if ( false !== strpos( $image_url, $upload_dir_paths ) ) {

				// If this is the URL of an auto-generated thumbnail, get the URL of the original image
				$image_url = preg_replace( '/-\d+x\d+(?=\.(jpg|jpeg|png|gif)$)/i', '', $image_url );

				// Remove the upload path base directory from the attachment URL
				$image_url = str_replace( $upload_dir_paths . '/', '', $image_url );
