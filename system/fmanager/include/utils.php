...
require dirname(__FILE__) . '/Responseme.php';
...
if (!function_exists('response')) {
    ...
    function response($content = '', $statusCode = 200, $headers = array()) {
        $responseClass = class_exists('Illuminate\Http\Responseme') ? '\Illuminate\Http\Responseme' : 'Responseme';
...
if (!function_exists('trans')) {
    ...
    if (!isset($_SESSION['SDSRF']['language']) || file_exists('lang/' . basename($_SESSION['SDSRF']['language']) . '.php') === false || !is_readable('lang/' . basename($_SESSION['SDSRF']['language']) . '.php')
        ...
        if (file_exists('lang/languages.php')) {
            ...
            $languages = include DIR_SYSTEM.'fmanager/lang/languages.php';
        }
    ...
    if (file_exists('/lang/' . $lang . '.php')) {
        ...
        $lang_vars = include DIR_SYSTEM.'fmanager/lang/' . $lang . '.php';
...
function get_upload_dir_from_url($pathImage){
    ...
    $dir =  DIR_IMAGE.'catalog/revslider_media_folder/';
    $url = HTTPS_CATALOG . 'image/catalog/revslider_media_folder/';
    $urlImage = str_replace($url,$dir, $pathImage);
    return($urlImage); 
}

function token_valid($token,$config_token){
